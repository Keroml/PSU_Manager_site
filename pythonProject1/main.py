...
import os
import codecs
from flask import Flask, render_template, url_for, request, session, redirect, abort, g, flash, make_response, send_file, Response, send_from_directory
...
#print( url_for('оброботчик(имя функции в код вызывается') ) возвращает url адрес
app=Flask(__name__)
...
def connect_db():#создание бд\установление соединения
...
def showPost(id_post):
    ...
    if not classes or not themes:
        abort(404)
    return render_template('post.html', menu=dbase.getMenu(), classes=classes, title= themes, post=post,id_post=id_post)
...
@app.route("/post1/<int:id_post>")
@login_required
def post(id_post):
    result = dbase.getPost(id_post)
    if result:
        x1 = result[0]
        x2 = result[1]
        with open(f"static/{x1}-{x2}.doc", 'w') as file:
            file.writelines([result[2]])
        directory = 'static'
        # Имя файла, который нужно скачать
        filename = f"{x1}-{x2}.doc"
        # Отправляем файл пользователю
        return send_from_directory(directory, filename, as_attachment=True)

    else:
        flash("Пост с указанным ID не найден.", "success")
        return render_template('page404.html', title="Страница не найдена", menu=dbase.getMenu()), 404
